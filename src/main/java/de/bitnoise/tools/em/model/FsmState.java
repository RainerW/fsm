...
public class FsmState {
	...
	public FsmState addTransition(String nextState, String[] events,
			EMAction[] actions) {
	...
	public FsmState addTransition(String nextState, String event,
			EMAction... actions) {
		FsmTransition transition = _transitions.get(event);
		if (transition != null) {
			if (!transition.getNextState().equals(nextState)) {
				throw new IllegalStateException(
						"the existring transision had the state "
